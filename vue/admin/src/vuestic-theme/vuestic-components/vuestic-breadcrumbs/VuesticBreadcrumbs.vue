  ...
  <div class="vuestic-breadcrumbs">
    <div class="vuestic-breadcrumbs__nav-section">
      <router-link
        ...
        :to="{ path: displayedCrumbs.root.name }">
        {{ $t(displayedCrumbs.root.displayName) }}
      </router-link>
      <router-link
        v-for="(item, index) in displayedCrumbs.routes"
        :to="{ name: item.name }"
        :key="index"
        class="vuestic-breadcrumbs__nav-section-item"
...
export default {
  ...
  computed: {
    displayedCrumbs () {
      const crumbs = utils.findInNestedByName(this.breadcrumbs, this.currentPath)
